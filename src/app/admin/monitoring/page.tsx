  ...
  return (
    <div className="p-6 max-w-[1100px] animate-fade-in">
      ...
      {loadingOverview ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard label="Total Hotels" value={overview.totalHotels} sub={`${overview.activeHotels} active`} accent="gold"
          ...
          />
          <StatCard label="Total Guests" value={overview.totalGuests} accent="teal"
          ...
          />
          <StatCard label="SMS Delivered" value={overview.smsStats.sent} sub={`${overview.smsStats.failed} failed`} accent="blue"
          ...
          />
          <StatCard
            label="PMS Sync Issues"
            value={overview.pmsSync.withErrors}
            sub={`${overview.pmsSync.active} active integrations`}
      ...
      {(overview?.pmsSync?.lastErrors?.length ?? 0) > 0 && (
        <div className="card mb-4">
          <div className="px-5 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            <h2 className="font-display font-700 text-sm text-white">PMS Sync Issues</h2>
          </div>
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <h2 className="font-display font-700 text-sm text-white">Recent SMS Failures</h2>
          {overview && overview.smsStats.failed > 0 && (
            <span className="badge" style={{ color: '#F43F5E', background: 'rgba(244,63,94,0.12)' }}>
              {overview.smsStats.failed} failed
            </span>
          )}
        </div>
        ...
        {loadingErrors ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead>
                <tr><th>Hotel</th><th>Phone</th><th>Template</th><th>Provider</th><th>Error</th><th>Time</th></tr>
              </thead>
              <tbody>
